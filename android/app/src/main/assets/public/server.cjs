...
function generateFallbackStudyPlan(studentName, subject, topics, examDate, daysAvailable, hoursPerDay, difficulty, topicDifficulty, dailyTarget, preferredTime, notes) {
...
function generateFallbackAssistantResponse(mode, subject, input, options) {
  ...
  if (mode === "summarize") {
    return `### \u{1F4CC} Executive Summary: ${subject}
...
### \u{1F4A1} High-Yield Exam Checklist
...
3. **Common Trap:** Avoid confusing dependent variables with independent constants.`;
  ...
  if (mode === "explain" || mode === "simple_explain") {
    return `### \u{1F3AF} Simple Explanation: ${input}
...
Remember **F-O-C-U-S**: **F**oundation $\\rightarrow$ **O**peration $\\rightarrow$ **C**heck units $\\rightarrow$ **U**nify terms $\\rightarrow$ **S**tate answer!`;
  ...
  if (mode === "revision_notes") {
    return `# \u{1F4DD} FocusGuard Master Revision Notes: ${subject}
